...
<header class="w-screen h-20 z-20 relative font-semibold text-gray-800">
    <nav class={`${background} text-black p-2 flex justify-between align-baseline
        ...
        <div data-testid="mobile-nav"  class="md:hidden flex right-group gap-2">
            <a href="/contact" class="btn rounded-lg btn-md variant-filled-primary flex-grow-0 text-sm font-bold" data-sveltekit-preload-data="hover">{$t('header.consult')}</a>
        ...
        <div data-testid="wide-screen-nav" class="hidden md:flex links gap-4
            ...
            <a href="/emergency">{$t('header.nav.emergency')}</a>
            <a href="/contact" class="btn rounded-lg btn-md variant-filled-primary flex-grow-0 text-sm font-bold" data-sveltekit-preload-data="hover">{$t('header.consult')}</a>
        </div>
    </nav>
    ...
    {#if showMobileNav}
        <div data-testid="nav-dropdown" class="w-screen flex flex-col
                    md:hidden">
            <a class="dropdown-link bg-surface-300 hover:bg-surface-400" href="/">{$t('header.nav.home')}</a>
            <a class="dropdown-link bg-surface-300 hover:bg-surface-400" href="/about">{$t('header.nav.about')}</a>
            <a class="dropdown-link bg-surface-300 hover:bg-surface-400" href="/services">{$t('header.nav.services')}</a>
            <a class="dropdown-link bg-surface-300 hover:bg-surface-400" href="/emergency">{$t('header.nav.emergency')}</a>
        </div>
    {/if}
</header>
